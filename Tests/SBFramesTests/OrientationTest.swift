...
class OrientationTest: XCTestCase {
  ...
  override func setUp() {
    ...
    XCTAssertEqualWithAccuracy(Double( angle), 0.0, accuracy: 1e-5)
    XCTAssertEqualWithAccuracy(Double(axis.x), 0.0, accuracy: 1e-5)
    XCTAssertEqualWithAccuracy(Double(axis.y), 0.0, accuracy: 1e-5)
    XCTAssertEqualWithAccuracy(Double(axis.z), 0.0, accuracy: 1e-3)
  }
  ...
  func checkValues (_ p: Position, frame: Frame, unit: SBUnits.Unit<Length>, x: Double, y: Double, z: Double) {
    ...
    XCTAssertEqualWithAccuracy(p.z.value, z, accuracy: accuracy)
  ...
  func checkOrientation (_ o: Orientation, frame: Frame, a: Double, x: Double, y: Double, z: Double) {    
    ...
    XCTAssertEqualWithAccuracy(ao, a, accuracy: accuracy)
    XCTAssertEqualWithAccuracy(xo, x, accuracy: accuracy)
    XCTAssertEqualWithAccuracy(yo, y, accuracy: accuracy)
    XCTAssertEqualWithAccuracy(zo, z, accuracy: accuracy)
    
    XCTAssert(o.has (frame: frame))
  }

  func checkQuant (qA: Quaternion, qB: Quaternion) {
    XCTAssertEqualWithAccuracy(qA.q0, qB.q0, accuracy: accuracy)
    XCTAssertEqualWithAccuracy(qA.q1, qB.q1, accuracy: accuracy)
    XCTAssertEqualWithAccuracy(qA.q2, qB.q2, accuracy: accuracy)
    XCTAssertEqualWithAccuracy(qA.q3, qB.q3, accuracy: accuracy)
  }
  ...
  func testRotate () {
    ...
    XCTAssertEqualWithAccuracy(c90.quat.q0, o90.quat.q0, accuracy: 1e-10)
  
    let c180 = o90.compose(o90)
    let o180 = Orientation (frame: base, angle: a180, axis: .z)
    XCTAssertEqualWithAccuracy(c180.quat.q0, o180.quat.q0, accuracy: 1e-10)
